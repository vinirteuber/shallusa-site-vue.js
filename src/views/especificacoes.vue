...
export default {
  ...
  methods: {
    async getAllEmpreendimentos(id) {
      await this.$get(
        `especificacoes/dolar/${id}/`,
        this.especificacoes.empreendimentos
      );
    },
    async removerDolar() {
    ...
    async postEspecificacoes() {
      this.especificacoes.empreendimentos = this.$route.params.id;
      await this.$post("dolar/", this.especificacoes);
  ...
  computed: {
    ...mapState(useAuthStore, ["empreendimentos", "username", "is_superuser"]),
  },
};
</script>
