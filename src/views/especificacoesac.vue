...
export default {
  ...
  methods: {
    async getAllacoes(id) {
      await this.$get(
        `especificacoesac/acoes/${id}/`,
        this.especificacoes.acoes
      );
    },
    async removerDolar() {
      await axios.delete(`http://localhost:8000/acoes/${this.id}/`);
      alert("acoes deletado com sucesso!"), this.$router.push("/");
    },
    async postEspecificacoes() {
      this.especificacoes.acoes = this.$route.params.id;
      await this.$post("acoes/", this.especificacoes);
  ...
  computed: {
    ...mapState(useAuthStore, ["acoes", "username", "is_superuser"]),
  },
};
</script>
